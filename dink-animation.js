// Dinking Consistency — Isometric court animation
// Renders into a canvas inside a given container element.

(function () {
    const container = document.getElementById('dink-container');
    if (!container) return;
    ...
    function resize() {
        ...
        width = rect.width;
        height = width * 0.85; // aspect ratio
    ...
    function planNextShot() {
        ...
        ball.progress = 0;
        ball.state = 'flying';
        var isBottomTurning = ball.startY > 0;
        var nextYDir = isBottomTurning ? -1 : 1;
        ball.bounceX = (Math.random() - 0.5) * courtWidth * 1.6;
        ball.bounceY = nextYDir * (Math.random() * (kitchenLen - 20) + 10);
        var nextSide = isBottomTurning ? 'top' : 'bottom';
        var opponentKitchenLineY = nextYDir * (kitchenLen + 12);
        ...
        var minH = (netHeight + 12) / (4 * ball.tNet * (1 - ball.tNet));
    ...
    function drawTraces() {
        ...
        ctx.strokeStyle = 'rgba(239,255,0,0.4)'; ctx.lineWidth = 2;
        ctx.beginPath();
    ...
    function drawBall() {
        ...
        var size = ball.radius + (ball.z * 0.05);
